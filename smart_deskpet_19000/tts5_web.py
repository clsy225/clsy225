...
APP_DIR = Path('/userdata')
DATA_DIR = APP_DIR / '.tmp' / 'tts5-webui'
...
REMOTE_EMOTION_HOST = os.environ.get('REMOTE_EMOTION_HOST', '192.168.137.1')
REMOTE_EMOTION_PORT = int(os.environ.get('REMOTE_EMOTION_PORT', '9998'))
REMOTE_EMOTION_TIMEOUT = float(os.environ.get('REMOTE_EMOTION_TIMEOUT', '20'))
...
def send_audio_to_remote_emotion_server(audio_bytes: bytes, host: str = REMOTE_EMOTION_HOST, port: int = REMOTE_EMOTION_PORT, timeout: float = REMOTE_EMOTION_TIMEOUT):
    if not audio_bytes:
    ...
    print(f'🎭 远程情绪判断返回: {response}', flush=True)
    return {
        'ok': True,
        'remote_host': host,
        'remote_port': port,
...
<div class="wrap">
  ...
  <div class="row">
    <div class="col card">
      <h2>9) 远程情绪判断（电脑 9998）</h2>
      <label>上位机 IP</label>
      <input id="remoteEmotionHost" value="192.168.137.1" />
      <label>上位机端口</label>
      <input id="remoteEmotionPort" type="number" value="9998" />
      <label>上传音频到上位机判断</label>
      <input id="emotionFile" type="file" accept=".wav,.flac,.mp3,.m4a,.ogg,.webm" />
      <button id="emotionFileBtn" onclick="runRemoteEmotionByFile()">上传音频并判断情绪</button>
      <div class="muted">也可以直接用板子本机麦克风录一段再发给电脑上的程序。</div>
      <label>本机录音秒数</label>
      <input id="emotionCaptureSeconds" type="number" value="4" min="1" max="20" />
      <label>本机录音设备</label>
      <select id="emotionCaptureDevice"></select>
      <button id="emotionHostBtn" onclick="recordHostEmotionRemote()">本机录音并发给上位机</button>
      <pre id="emotionResult">等待情绪判断...</pre>
    </div>
  </div>
</div>
...
async function refreshStatus() {
  try {
    ...
    const sels = [
      document.getElementById('captureDevice'),
      document.getElementById('queryCaptureDevice'),
      document.getElementById('emotionCaptureDevice')
    ];
...
async function recordHostRef() {
  ...
  try {
    ...
    showToast('板子本机录参考失败：录音后端异常，请刷新页面后重试。', 'err', 4200);
  } finally {
...
async function recordHostQueryAndIdentify() {
  try {
    ...
    showToast('板子本机录音识别失败：录音后端异常，请刷新页面后重试。', 'err', 4200);
  } finally {
    setBtnBusy('hostQueryBtn', false);
  }
}

async function runRemoteEmotionByFile() {
  const file = document.getElementById('emotionFile').files[0];
  if (!file) return alert('先选择一段音频');
  const fd = new FormData();
  fd.append('file', file);
  fd.append('host', document.getElementById('remoteEmotionHost').value || '192.168.137.1');
  fd.append('port', document.getElementById('remoteEmotionPort').value || '9998');
  try {
    showToast('正在把音频发到上位机做情绪判断...', 'warn', 2000);
    setBtnBusy('emotionFileBtn', true, '发送中...');
    const data = await jsonFetch('/api/emotion_remote', { method:'POST', body: fd });
    document.getElementById('emotionResult').textContent = JSON.stringify(data, null, 2);
    showToast('远程情绪判断已返回：' + (data.response || '无结果'), 'ok', 3600);
  } catch (e) {
    document.getElementById('emotionResult').textContent = '远程情绪判断失败: ' + e.message;
    showToast('远程情绪判断失败：' + e.message, 'err', 4200);
  } finally {
    setBtnBusy('emotionFileBtn', false);
  }
}

async function recordHostEmotionRemote() {
  try {
    showToast('板子本机开始录音，并发给上位机判断情绪...', 'warn', 2200);
    setBtnBusy('emotionHostBtn', true, '录音并发送中...');
    const data = await jsonFetch('/api/record_host_emotion_remote', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify({
        device: document.getElementById('emotionCaptureDevice').value,
        seconds: parseInt(document.getElementById('emotionCaptureSeconds').value || '4', 10),
        host: document.getElementById('remoteEmotionHost').value || '192.168.137.1',
        port: parseInt(document.getElementById('remoteEmotionPort').value || '9998', 10),
      })
    });
    document.getElementById('emotionResult').textContent = JSON.stringify(data, null, 2);
    showToast('远程情绪判断已返回：' + (data.response || '无结果'), 'ok', 3600);
  } catch (e) {
    document.getElementById('emotionResult').textContent = '本机录音远程情绪判断失败: ' + e.message;
    showToast('本机录音远程情绪判断失败：' + e.message, 'err', 4200);
  } finally {
    setBtnBusy('emotionHostBtn', false);
  }
}
...
def api_emotion_remote():
    ...
    host = (request.form.get('host') or REMOTE_EMOTION_HOST).strip()
    port = int(request.form.get('port') or REMOTE_EMOTION_PORT)
    audio_bytes = f.read()
    result = send_audio_to_remote_emotion_server(audio_bytes, host=host, port=port)
...
def api_record_host_emotion_remote():
    ...
    seconds = max(1, min(20, int(data.get('seconds', 4))))
    device = (data.get('device') or DEFAULT_CAPTURE_DEVICE).strip()
    host = (data.get('host') or REMOTE_EMOTION_HOST).strip()
    port = int(data.get('port') or REMOTE_EMOTION_PORT)

    ts = datetime.utcnow().strftime('%Y%m%dT%H%M%S%f')
    out = QUERY_DIR / f'{ts}_host_emotion.wav'
    rec = run_capture(seconds, out, device)
    if rec['returncode'] != 0:
        return jsonify({'error': 'arecord failed', 'record': rec}), 500
    audio_bytes = out.read_bytes()
    result = send_audio_to_remote_emotion_server(audio_bytes, host=host, port=port)
    return jsonify({'record': rec, 'file': str(out), **result})
